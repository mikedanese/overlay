...
// actions
int
exec_action_helper(overlayer* o, json_t* args, bool host) {
	pid_t pid = fork();
	if (pid) {
		int status = 0;
...
int
do_exec_action(overlayer* o, json_t* args) {
	return exec_action_helper(o, args, false);
}

int
do_host_exec_action(overlayer* o, json_t* args) {
...
int
register_action(struct overlay_action action) {
	int i;
